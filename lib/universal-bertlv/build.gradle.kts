...
tasks.register<Copy>("copyAndroidNativeLibs") {
    ...
    val pluginJniLibs = rootProject.projectDir.resolve(
        "../plugin/universal_bertlv_mobile/android/src/main/jniLibs",
    )
    val buildBin = layout.buildDirectory.dir("bin").get().asFile

    from("$buildBin/androidNativeArm64/releaseShared/libuniversal_bertlv.so") {
        into("arm64-v8a")
    }
    from("$buildBin/androidNativeX64/releaseShared/libuniversal_bertlv.so") {
        into("x86_64")
    }
    from("$buildBin/androidNativeArm32/releaseShared/libuniversal_bertlv.so") {
        into("armeabi-v7a")
    }
...
tasks.register<Copy>("copyJsToWebPlugin") {
    description = "Copy JS production distribution to Flutter universal_bertlv_web plugin"
    group = "distribution"
    ...
    val webAssetsDir = rootProject.projectDir.resolve(
        "../plugin/universal_bertlv_web/web/assets/js",
    )
...
mavenPublishing {
    ...
    coordinates("io.github.rafaelrabeloit", "universal-bertlv", version.toString())
